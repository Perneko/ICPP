...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 19 14:31:20 2018
...
def zScaleFeatures(vals):
    '''
    Assumes vals in a sequence of floats
    '''
...
def readMammalData(fName, scale):
    ...
    for mammal in range(len(species_names)):
        featureVector = []
        for feature in range(len(species_names)):
            featureVector.append(feature_vals[feature][mammal])
        featureVectorList.append(featureVector)
    return featureVectorList, label_list, species_names

def testTeeth(num_clusters, num_trials, scale = lambda x: x):
